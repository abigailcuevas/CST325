...
function createScene() {
    ...
    var rotation = new Matrix4().makeRotationX(-89.8);
    var scale = new Matrix4().makeScale(10, 10, 10);
    groundGeometry.worldMatrix.multiply(rotation);
    groundGeometry.worldMatrix.multiply(scale);

    // todo #3 - create the sphere geometry DONE
    sphereGeometry = new WebGLGeometryJSON(gl);
    sphereGeometry.create(loadedAssets.sphereJSON);

    // todo #4 - scale and translate the sphere DONE
    var first = new Matrix4().makeScale(.01,.01,.01);
    sphereGeometry.worldMatrix.makeTranslation(0,0,-5);
    sphereGeometry.worldMatrix.multiply(first);
...
function updateAndRender() {
    ...
    // todo #4 - change color for the sphere DONE
    // todo #9 - animate the color of there sphere DONE
    // todo #10 - animate the color with non-grayscale values DONE

    var c = Math.cos(time.secondsElapsedSinceStart);
    var d = Math.sin(time.secondsElapsedSinceStart);
    gl.uniform4f(colorProgram.uniforms.colorUniform, c, 1.0-d, d, 1.0);

    
    // todo #3 - render the sphere DONE
