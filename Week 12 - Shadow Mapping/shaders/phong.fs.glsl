...
//-----------------------------------------------------------------------------------
    ...
    //DONE
    //vec4 sampleTexture = texture2D(uShadowTexture, vTexCoords);
...
//-----------------------------------------------------------------------------------
    ...
    float xCoord = (lightSpaceNDC.x + 1.0)/2.0; // [-1,1] = [0,2] = [0,1]
    float yCoord = (lightSpaceNDC.y + 1.0)/2.0;

    // scale and bias the light-space NDC xy coordinates from [-1, 1] to [0, 1]
    vec2 lightSpaceUV = vec2(xCoord, yCoord);
